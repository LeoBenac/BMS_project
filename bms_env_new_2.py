...
class BMSenv(gym.Env):
    ...
    def __init__(self, num_cells: int = 2,  k_tanh_params : list = [k_default, k_default],
                   ...
                   var_weight = .4, max_min_weight= .3, switch_weight= .3, w_max = 1.0,
                   bins =  np.array([0.01, 0.02, 0.05, 0.1, 0.15, 0.2, 0.5])):
        ...
        self.action_space = spaces.Discrete(len(bins))
    ...
    def int_action_to_switch_action(self, action):


        state_soc = self.state[:self.num_cells].copy()
        min_soc = np.min(state_soc)
        percentage = self.bins[action]


        switch_action = np.array([1 if soc > (min_soc  + percentage) else 0 for soc in state_soc])
        
        # percentage = self.bins[action]
        # switch_action = np.zeros(self.num_cells)
        # for i in range(self.num_cells):
        #     if state_soc[i] > percentage:
        #         switch_action[i] = 1
    ...
    def step(self, action) -> tuple:
        ...
        if type(action) != list and type(action) != tuple and type (action) != np.ndarray:  
            switch_action = self.int_action_to_switch_action(action)
        else:
            switch_action = action.copy()

        self.charge(switch_action)
        ...
        reward = self.get_reward(state, switch_action, state_next)
